...
if(isset($_POST['btc'])){
	...
	$temp->status = call_user_func(function() use(&$msg, $__user, $link, $temp) {
		...
		if( (float)$data['moni'] > (float)$__user['walletbalance'] ) return !($msg = "Insufficient account balance");
		...
		foreach( $queries as $sql ) {
			if( !$link->query($sql) ) {
				return !($msg = "
					...
					Please try again or send a support ticket for assistance
				");
			};
		};
		...
		$msg = "
			...
			Your account will be credited once the request is confirmed.
		";
